...
bool SkinnedShader::Init(VulkanDevice * vulkanDevice)
{
	VkResult result;
	...
	// Vertex shader
	FILE * file = NULL;
	file = fopen("data/shaders/skinnedVS.spv", "rb");
	if (file == NULL)
	{
		gLogManager->AddMessage("ERROR: Couldn't find vertex shader file: skinnedVS.spv");
		return false;
	}
	...
	// Fragment shader
	file = fopen("data/shaders/skinnedFS.spv", "rb");
	if (file == NULL)
	{
		gLogManager->AddMessage("ERROR: Couldn't find vertex shader file: skinnedFS.spv");
		return false;
	}
...
void SkinnedShader::Unload(VulkanDevice * vulkanDevice)
{
...
VkPipelineShaderStageCreateInfo * SkinnedShader::GetShaderStages()
{
	return shaderStages;
}
