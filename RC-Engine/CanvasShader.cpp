...
bool CanvasShader::Init(VulkanDevice * vulkanDevice)
{
	VkResult result;
	...
	// Vertex shader
	FILE * file = NULL;
	file = fopen("data/shaders/canvasVS.spv", "rb");
	if (file == NULL)
	{
		gLogManager->AddMessage("ERROR: Couldn't find vertex shader file: canvasVS.spv");
		return false;
	}
	...
	// Fragment shader
	file = fopen("data/shaders/canvasFS.spv", "rb");
	if (file == NULL)
	{
		gLogManager->AddMessage("ERROR: Couldn't find vertex shader file: canvasFS.spv");
		return false;
	}
...
void CanvasShader::Unload(VulkanDevice * vulkanDevice)
{
...
VkPipelineShaderStageCreateInfo * CanvasShader::GetShaderStages()
{
	return shaderStages;
}
